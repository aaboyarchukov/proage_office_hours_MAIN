...
const Submit = () => {
    ...
    useEffect(() => {
        ...
        if (data && selectedId !== 99) {
            const correctData = data.slots.find((el) => el.id === selectedId);
            setTeacher(correctData.teacher);
            setTime(correctData.datetime);
            console.log(data)
    ...
    async function fetchSubmit() {
        ...
        if (timeDate.toDateString() === currentTime.toDateString()) {
            const webApp = window.Telegram.WebApp;
            webApp.sendData(`book|today|${selectedId}|${question}`);
        } else {
    ...
    if ((!teacher && !time) || isLoading) return <Loader/>
        ...
        <>
            ...
            <main>
                <div className={styles.teacherSubmit__container}>
                    ...
                    <TeacherSubmitCard teacher={teacher}/>
                </div>
                <div className={styles.timeSubmit__container}>
                    <h2 className={styles.submit__title}>Дата и время</h2>
                    <TimeSubmitCard date={time} course={selectedModule.name}/>
                </div>

                <div className={styles.question__container}>
                    <h2 className={styles.submit__title}>Вопрос для занятия</h2>
                    <input
                        type="text"
                        value={question}
                        onChange={(e) => {
                        ...
                        className={`${styles.question__textarea} ${questionError ? styles.error : ""}`}
                        placeholder="Напишите здесь вопрос преподавателю"
                        required
                    />
                </div>

                <p style={{marginBottom: '10px', marginTop: '30px', fontSize: '15px'}}>
                    Занятие можно отменить за <span style={{fontWeight: 'bold'}}>3 часа</span> до его начала, нажав на кнопку <span
                    style={{fontWeight: 'bold'}}>Отменить</span> в главном меню. Если ученик не присутствует на занятии, и оно не было отменено, то занятие считается проведенным.
                </p>

                <div className={styles.submit__end}>
                    <button onClick={async () => handleSubmit()} className={styles.submit__button}>Подтвердить</button>
                </div>
...
const TeacherSubmitCard = ({teacher}) => {
    return (
        <div className={styles.teacherSubmitCard}>
            ...
            <Link className={styles.TeacherSubmitCard__icon} href={"/teachers"}><EditIcon/></Link>
        </div>
    )
}
    ...
    return (
        <div className={styles.timeSubmitCard}>
            <div className={styles.timeSubmitCard__container}>
                ...
                <div className={styles.timeSubmitCard__duration}>длительность: 40 минут</div>
                <div className={styles.timeSubmitCard__course}>{courseName}</div>
            </div>
            <Link className={styles.timeSubmitCard__icon} href={"/timeSlot"}><EditIcon/></Link>
        </div>
    )
}
